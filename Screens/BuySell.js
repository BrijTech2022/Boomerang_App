...
const { width, height } = Dimensions.get('window');
  ...
  class BuySellView extends React.PureComponent {
    ...
    componentDidMount(){
      ...
      // {
        this.setState({
          // liverate:Math.abs(this.props.route.params.data.liverate.toFixed(2)),
          participation_id:this.props.route.params.data.id,
          // totalUnit:Number(this.props.route.params.data.totalUnit)
    ...
    ParticipationDetail=()=>{
      ContestApi.GetDetailsByParticipationId(this.state.participation_id).then((data)=>{
        ...
        console.log("participationBuy====>>"+JSON.stringify(data))
      })
    }
    ...
    checkAmount=(txt)=>{
        ...
        if(this.state.totalUnit>=Number(txt))
        {
          if((Number(this.state.liverate)*Number(txt))<=this.state.maxQtyAmount)
        {
          if(this.state.balance<=(Number(this.state.liverate)*Number(txt)))
          {
            this.setState({warning:true,errmsg:'Available amount is not enough',Qtywarning:false})
          }
          else
          {
            this.setState({warning:false,errmsg:'',Qtywarning:false})
          }
        }
        else{
          this.setState({Qtywarning:true,errmsg:"You can't buy more than ₹"+this.state.maxQtyAmount,warning:false})
        }
        }
        else{
          this.setState({Qtywarning:true,errmsg:'Qty is not enough you can buy max:'+this.state.totalUnit+' qty',warning:false})
        }
    }
    ...
    BuyStock=()=>{
      if(this.state.qty==0)
      {
        this.setState({warning:true,errmsg:'Quantity should be more than 0',Qtywarning:false})
      }
      else
      {
        if(this.state.needAmount>0)
        {
          
        if(this.state.totalUnit>=this.state.qty)
        {
          if(this.state.needAmount<=this.state.maxQtyAmount)
          {
            this.setState({isLoading:true})        
            let wData={
              "fkUserId":this.props.uInfo.userId
            ...
            TransactionApi.GetWalletAmountByUserId(wData).then((data)=>{
              ...
              if(data.data.length!=0)
              {
                if(Number(data.data[0].amount)>=this.state.needAmount.toFixed(2))
                {
                // alert(data.data[0].amount)
                let TData={
                  "transactionId": 0,
                  "participation_id": Number(this.state.participation_id),
                  // "amount": -Number(this.state.needAmount.toFixed(2)),
                ...
                TransactionApi.SaveTransaction(TData).then((ReqTdata)=>{
                  ...
                  if(ReqTdata.data.length!=0)
                  {
                    ...
                    alert('Buy success')
                    this.RefreshData()
                    // this.props.navigation.replace('BuySellScreen',{data:this.props.route.params.data});
                ...
                {
                  this.setState({isLoading:false,balance:data.data[0].amount,warning:true,errmsg:'Available amount is not enough',Qtywarning:false})
                }
              }
              else
              {
                this.setState({warning:true,errmsg:'Available amount is not enough',isLoading:false,Qtywarning:false})
              }
            }
            else
            {
        ...
        {
          this.setState({Qtywarning:true,errmsg:'Required amount is not valid',warning:false})
        }
      }
    }
    ...
    BuySell=()=>{
        return(
         ...
         <ScrollView>
            ...
            {/* <Text style={{fontSize:20,}}>Image Doge</Text> */}
            <View style={{marginTop:5}}>
              <Text style={{fontSize:25,fontWeight:'bold',}}>₹ {this.state.liverate} <Text style={{fontSize:15,fontWeight:'bold',color:this.FindPercentage(this.state.baseAmount,this.state.liverate)>=0?'green':'red' }}>
                ( {this.FindPercentage(this.state.baseAmount,this.state.liverate)>=0?"+"+this.FindPercentage(this.state.baseAmount,this.state.liverate):this.FindPercentage(this.state.baseAmount,this.state.liverate)}% )
                </Text></Text>
            ...
            <View style={{marginTop:30,flexDirection:'row',justifyContent:'space-between'}}>
                ...
                <TextInput 
                    ...
                    onChangeText={(txt)=>{
                      ...
                      this.setState({qty:0,needAmount:0,warning:false,errmsg:'',Qtywarning:false})
                    }
                  }}
                />
                </View>
        ...
        <View>
          <View style={{width:'80%',backgroundColor:'#f38589',alignSelf:'center',marginBottom:dynamicSize(10),display:(this.state.warning==true || this.state.Qtywarning==true)?'flex':'none'}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between',paddingHorizontal:25,marginTop:5}}>
            <View>
                <Text style={{color:'gray'}}>Balance :  ₹{this.state.balance.toFixed(2)}</Text>
            </View>
            <View>
                <Text style={{color:'gray'}}>Required :  ₹{this.state.needAmount.toFixed(2)}</Text>
            </View>
        </View>
         <View style={{padding:20,backgroundColor:backColor}}>

            
         <View style={{width:width-50,alignSelf:'center',backgroundColor:'#2AAA8A',borderRadius:10}}>
                <TouchableOpacity
                  onPress={()=>{
                  ...
                  disabled={this.state.Qtywarning}
                >
                    <Text style={{padding:10,color:'white',fontSize:15,textAlign:'center'}}>{(this.state.warning==true && this.state.qty!=0)?"Add Money":"BUY"}</Text>  
                </TouchableOpacity>              
            </View>
        </View>
        </View>
    ...
    const mapStateToProps = (state) => {
      ...
      export default connect(mapStateToProps)(BuySellView)
